...
void Graficadora::setString(){
    ...
    for(int i = size ; i>=0;i--){
        if(Vpoli[i]==0){
            ...
            if(Vpoli[i]>0)
                cadena+= QString::fromStdString("+") + QVariant(Vpoli[i]).toString();
            else
                cadena+= QVariant(Vpoli[i]).toString();
        }
        else{
            if(i==size){
                if(Vpoli[i]==1 || Vpoli[i]==-1){
                    if(Vpoli[i]==1){
                    ...
                    cadena+=QVariant(Vpoli[i]).toString() + QString::fromStdString("x^") + QVariant(i).toString();
               }
            else{
                if(Vpoli[i]==1 || Vpoli[i]==-1){
                    if(Vpoli[i]==-1)
                    ...
                    cadena+= QString::fromStdString("+") + QVariant(Vpoli[i]).toString() + QString::fromStdString("x^") + QVariant(i).toString();
                else
                    cadena+= QVariant(Vpoli[i]).toString() + QString::fromStdString("x^") + QVariant(i).toString();
            }
        }
...
void Graficadora::makePlot(){
    ...
    for (int i=0;i<2*XMAX+1;++i){
        x[i]= (XMIN+i)/10.0;
        y1[i] = evaluar(Mpoli,x[i]);// let's plot a polinomic function
    }
