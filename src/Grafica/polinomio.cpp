...
polinomio::polinomio(string fct)
{
    Mpoli = split(fct);
    Vpoli = maptovector(Mpoli);
    result = horners();
}
...
inline vector<double> polinomio::maptovector(map<int, double> polinomio){
...
inline map<int, double> polinomio::vectortomap(vector<double> polinomio){
    map<int, double> poli;
...
*/
inline map<int, double> polinomio::derv(map<int, double> &polinomio){
...
*/
inline double polinomio::evaluar( double x){
    double g=0;
    map<int, double>::iterator iter;
    for(iter = Mpoli.begin(); iter != Mpoli.end(); iter++)
        g+=(double)iter->second*pow(x,iter->first);
    return g;
}
inline double polinomio::evaluar2(map<int, double> &polinomio, double x){
    double g=0;
    map<int, double>::iterator iter;
    for(iter = polinomio.begin(); iter != polinomio.end(); iter++)
        g+=(double)iter->second*pow(x,iter->first);
...
*/
inline double polinomio::newton(map<int, double> &polinomio, map<int, double> &polinomio2,double x){
    double y;
    int iter = 0;
    y=x;
    while(++iter < LIMIT){
        y -= (evaluar2(polinomio,y)/evaluar2(polinomio2,y));
        if(abs(evaluar2(polinomio,y))<EPSILON)
            break;
    }
    flag = iter != LIMIT;
    return y;
...
*/
inline map<int, double> polinomio::div(map<int,double> &polinomio, double div) {
 ...
 *
*/
inline vector<double> polinomio::horners(){
    vector<double> sizevector=maptovector(Mpoli);
...
inline map<int, double> polinomio::split(string s){
    bool flag=false;
    map<int, double> polinomio1;
    for (int i = 0; i < s.size(); i=0){
        ...
        if(s[i]=='^'){
            ...
            polinomio1[stoi(cat)]=stoi(cat2);
        }
        else if(flag){
            polinomio1[1]=stoi(cat2);
            flag=false;
        }
        else{
            polinomio1[0]=stoi(cat2);
        }
